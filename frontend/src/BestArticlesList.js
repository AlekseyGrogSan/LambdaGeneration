...
import { extractNameAndIcon, ProfileIcon, resolveProfileIconValue } from './profileIcons';
      ...
      if (response.ok) {
        ...
        const enriched = await Promise.all(rawArticles.map(async (art) => {
          ...
          const rawAuthorName =
            ...
            authorData.Name ||
            'Автор';
          const parsedAuthor = extractNameAndIcon(rawAuthorName);
          ...
          return {
            ...
            nickname: parsedAuthor.name,
            authorAvatar: authorData.pathAvatar || authorData.PathAvatar,
            authorProfileIcon: parsedAuthor.icon || resolveProfileIconValue(authorData),
  ...
  const renderList = () => (
    <Box sx={{ p: 1, maxHeight: '70vh', overflowY: 'auto' }}>
      ...
      {loading ? (
        ...
        <Typography sx={{ color: '#bdbdbd', p: 2, textAlign: 'center' }}>Нет статей</Typography>
      )}
    </Box>
  );
