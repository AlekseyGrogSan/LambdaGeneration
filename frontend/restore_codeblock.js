...
\;
    ...
    // Strip old highlight DOM effects if they are still there (the ones with block.textContent = codeText)
    ...
    }); // RUNS AFTER EVERY RENDER TO CATCH DOM WIPES
\;

    // Put it all together
    // Inject imports after import React
    content = content.replace(/import (.*?) from 'react';\r?\n/, \import  from 'react';\n\ + codeBlockStr + '\n' + norm + '\n');
    
    // Find where the old useEffect was, or insert the new one right before the return statement of the component
    // Typically we can insert before 'return (' inside the component
    let compRegex;
    ...
    if (match) {
        ...
        console.error('Could not find return injection point for ' + filePath);
    }
...
restoreCodeBlock('src/PostDetailPage.js');
restoreCodeBlock('src/PostCard.js');
